(function () {
  ...
  var radiusScale = d3.scaleSqrt().domain([45, 75]).range([10, 80]);

  // Default, unir todos
  var forceXCombine = d3.forceX(width / 2).strength(0.05);

  // Chileno o extranjero
  var forceXExtranjero = d3
    .forceX(function (d) {
      if (d.extranjero === "si") {
        return 300;
      } else {
  ...
  d3.queue().defer(d3.csv, "d3/tecnicos.csv").await(ready);
  ...
  function ready(error, datapoints) {
    ...
    d3.select("#ch-extranjero").on("click", function () {
      simulation.force("x", forceXExtranjero).alphaTarget(0.25).restart();
      console.log("Chileno o extranjero");
    });

    d3.select("#combine").on("click", function () {
      simulation.force("x", forceXCombine).alphaTarget(0.25).restart();
      console.log("Combine the bubbles");
    });

    d3.select("#titulos").on("click", function () {
      simulation.force("x", forceXTitulos).alphaTarget(0.25).restart();
